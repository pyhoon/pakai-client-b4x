...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	B4XPages.SetTitle(Me, "Pakai Client")
	#If B4J
...
Private Sub GetCategories
	Try
		...
		If Verbose Then
			...
			Wait For (sd) Complete (M As Map)
			If ContentType = "application/xml" Then
				Dim M2 As Map = M.Get("root")
				Dim R1 As Map = M2.Get("r")
				Dim Items As List = R1.Get("item")
			Else
				Dim Items As List = M.Get("r")
			End If
		Else
...
Private Sub GetProducts
	Try
		clvRecord.Clear
		If Verbose Then
			Dim sd As Object = SendData("GET", $"find/products-by-category_id/${CategoryId}"$, Null)
			Wait For (sd) Complete (M As Map)
			If ContentType = "application/xml" Then
				Dim M2 As Map = M.Get("root")
				Dim R1 As Map = M2.Get("r")
				If R1.Get("item") Is List Then
					Dim Items As List = R1.Get("item")
				Else
					Dim M3 As Map = R1.Get("item")
					Dim Items As List = Array(M3)
				End If
			Else
				Dim Items As List = M.Get("r")
			End If
...
Sub SendData (Method As String, EndPoint As String, Payload As Map) As ResumableSub
	Try
		...
		'Log(Link)
		If Method.EqualsIgnoreCase("POST") Or Method.EqualsIgnoreCase("PUT") Then
			If PayloadType = "application/xml" Then
				Dim M2X As Map2Xml
				M2X.Initialize
				Dim Text As String = M2X.MapToXml(Payload)
			Else
				Dim Text As String = Payload.As(JSON).ToString
			End If
		End If		
		indLoading.Show
		Dim j As HttpJob
		...
		If j.Success Then
			...
			If ContentType = "application/xml" Then
				Dim X2M As Xml2Map
				X2M.Initialize
				Dim Data As Map = X2M.Parse(j.GetString)
			Else
				Dim Data As Map = j.GetString.As(JSON).ToMap
			End If
			#If B4J
	...
	Try
		...
		If j.Success Then
			...
			If ContentType = "application/xml" Then
				Dim X2M As Xml2Map
				X2M.Initialize
				Dim M1 As Map = X2M.Parse(j.GetString)
				Dim Data As List = M1.Get("item")
			Else
